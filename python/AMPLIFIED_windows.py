...
DEFAULT_OUTPUT_SUBDIR = r"output_tables_51"
...
def process_library(library, batch, data_dir, output_dir, config, integrated_data):
...
def main():
    ...
    for library in library_types:
        ...
        process_library(library, args.batch_name, args.data_dir, args.output_dir, config, integrated_data)
        print(f"Finished processing for library type: {library}\n")
    
    if integrated_data:
        integrated_df = pd.concat(integrated_data, ignore_index=True)
        # Save integrated results in the batch directory
